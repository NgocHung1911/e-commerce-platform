...
function cpm_render_auth_modal()
{
    ...
    ob_start();
    $theme_template = locate_template('template-parts/cpm/auth-modal.php');
    if ($theme_template) {
        include $theme_template;
    } else {
        ?>
        <?php if (!doing_action('wp_footer')) : ?>
        <div id="cpm-auth-wrapper" class="inline-block font-sans">
            <?php if ($is_logged_in): ?>
                <!-- Trạng thái: Đã đăng nhập -->
                <div class="relative inline-block text-left group">
                    <button type="button"
                        class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/90 hover:bg-white text-slate-800 text-xs md:text-sm font-semibold transition-all duration-200 cursor-pointer border border-slate-200 shadow-sm">
                        <?php
                    ...
                    <div
                        class="absolute right-0 top-full mt-1 w-52 bg-white rounded-xl shadow-2xl border border-slate-100 py-1.5 hidden group-hover:block z-[99999] before:content-[''] before:absolute before:-top-3 before:left-0 before:w-full before:h-3">
                        <div class="px-4 py-2 border-b border-slate-100 bg-slate-50 rounded-t-xl">
                            ...
                            <p class="text-xs font-bold text-slate-800 truncate"><?php echo esc_html($current_user->user_email); ?>
                            </p>
                        </div>
                        <a href="<?php echo esc_url(home_url('/gio-hang/')); ?>"
                            class="flex items-center gap-2 px-4 py-2 text-xs font-semibold text-slate-700 hover:bg-blue-50 hover:text-blue-600 transition-colors">🛒
                            Giỏ hàng của tôi</a>
                        <a href="<?php echo esc_url(home_url('/lich-su-don-hang/')); ?>"
                            class="flex items-center gap-2 px-4 py-2 text-xs font-semibold text-slate-700 hover:bg-blue-50 hover:text-blue-600 transition-colors">📋
                            Đơn hàng của tôi</a>
                        <a href="<?php echo esc_url(admin_url('profile.php')); ?>"
                            class="flex items-center gap-2 px-4 py-2 text-xs font-semibold text-slate-700 hover:bg-blue-50 hover:text-blue-600 transition-colors">⚙️
                            Quản lý tài khoản</a>
                        <a href="<?php echo esc_url($logout_url); ?>"
                            class="flex items-center gap-2 px-4 py-2 text-xs font-bold text-red-600 hover:bg-red-50 transition-colors border-t border-slate-100 mt-1">🚪
                            Đăng xuất</a>
                    </div>
                </div>
            <?php else: ?>
                <!-- Trạng thái: Chưa đăng nhập -->
                <button type="button" onclick="openCpmAuthModal('login')"
                    class="inline-flex items-center gap-2 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white text-xs md:text-sm font-bold px-4 py-2 rounded-xl shadow-md hover:shadow-lg transition-all duration-200 cursor-pointer border-none">
        ...
        <div id="cpmAuthModal" style="display: none;"
            class="fixed inset-0 z-[99999] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 animate-fadeIn">
            <div
                class="bg-white w-full max-w-md rounded-2xl shadow-2xl border border-slate-100 overflow-hidden relative transform transition-all my-auto mx-auto max-h-[90vh] overflow-y-auto">
                <!-- Nút Đóng Modal -->
                <button type="button" onclick="closeCpmAuthModal()"
                    class="absolute top-4 right-4 text-slate-400 hover:text-slate-600 w-8 h-8 rounded-full flex items-center justify-center hover:bg-slate-100 transition-colors cursor-pointer border-none">
                    ✕
                </button>

                <!-- Header Modal Tabs -->
                <div class="flex border-b border-slate-100 bg-slate-50 p-2 gap-2">
                    <button type="button" id="cpmTabBtnLogin" onclick="switchCpmAuthTab('login')"
                        class="flex-1 py-2.5 text-sm font-bold rounded-xl transition-all border-none cursor-pointer bg-white text-blue-600 shadow-sm">
                        Đăng nhập
                    </button>
                    <button type="button" id="cpmTabBtnRegister" onclick="switchCpmAuthTab('register')"
                        class="flex-1 py-2.5 text-sm font-semibold text-slate-500 hover:text-slate-800 rounded-xl transition-all border-none cursor-pointer">
                        Tạo tài khoản
                    </button>
                </div>
                ...
                <div class="p-6 md:p-8">
                    ...
                    <form id="cpmLoginForm" onsubmit="handleCpmLogin(event)">
                        <div class="space-y-4">
                            <div>
                                ...
                                <input type="text" id="cpmLoginEmail" required placeholder="example@gmail.com"
                                    class="w-full px-3.5 py-2.5 rounded-xl border border-slate-300 focus:border-blue-500 focus:ring-2 focus:ring-blue-100 text-sm outline-none transition-all box-border" />
                            </div>
                            <div>
                                <label class="block text-xs font-bold text-slate-700 mb-1">Mật khẩu</label>
                                <input type="password" id="cpmLoginPwd" required placeholder="••••••••"
                                    class="w-full px-3.5 py-2.5 rounded-xl border border-slate-300 focus:border-blue-500 focus:ring-2 focus:ring-blue-100 text-sm outline-none transition-all box-border" />
                            </div>
                            <div class="flex items-center justify-between text-xs">
                                <label class="flex items-center gap-1.5 text-slate-600 cursor-pointer">
                                    <input type="checkbox" id="cpmRememberMe"
                                        class="rounded border-slate-300 text-blue-600 focus:ring-blue-500" />
                                    <span>Ghi nhớ tôi</span>
                                </label>
                            </div>
                            <button type="submit" id="cpmSubmitLoginBtn"
                                class="w-full py-3 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white font-bold text-sm rounded-xl shadow-lg hover:shadow-xl transition-all border-none cursor-pointer">
                                Đăng nhập ngay
                            </button>
                        </div>
                    </form>
                    ...
                    <form id="cpmRegisterForm" class="hidden" onsubmit="handleCpmRegister(event)">
                        <div class="space-y-4">
                            <div>
                                ...
                                <input type="text" id="cpmRegName" required placeholder="Nguyễn Văn A"
                                    class="w-full px-3.5 py-2.5 rounded-xl border border-slate-300 focus:border-blue-500 focus:ring-2 focus:ring-blue-100 text-sm outline-none transition-all box-border" />
                            </div>
                            <div>
                                <label class="block text-xs font-bold text-slate-700 mb-1">Địa chỉ Email (Gmail)</label>
                                <input type="email" id="cpmRegEmail" required placeholder="tenban@gmail.com"
                                    class="w-full px-3.5 py-2.5 rounded-xl border border-slate-300 focus:border-blue-500 focus:ring-2 focus:ring-blue-100 text-sm outline-none transition-all box-border" />
                            </div>
                            <div>
                                <label class="block text-xs font-bold text-slate-700 mb-1">Tạo Mật khẩu (tối thiểu 6 ký
                                    tự)</label>
                                <input type="password" id="cpmRegPwd" required minlength="6" placeholder="••••••••"
                                    class="w-full px-3.5 py-2.5 rounded-xl border border-slate-300 focus:border-blue-500 focus:ring-2 focus:ring-blue-100 text-sm outline-none transition-all box-border" />
                            </div>
                            <button type="submit" id="cpmSubmitRegBtn"
                                class="w-full py-3 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700 text-white font-bold text-sm rounded-xl shadow-lg hover:shadow-xl transition-all border-none cursor-pointer">
                                Đăng ký tài khoản
                            </button>
                        </div>
                    </form>
                    ...
                    <div class="flex flex-col gap-2">
                        <button type="button" onclick="triggerGoogleOneTapLogin()"
                            class="w-full py-2.5 px-4 bg-white hover:bg-slate-50 text-slate-700 font-bold text-xs md:text-sm rounded-xl border border-slate-300 shadow-sm flex items-center justify-center gap-2.5 transition-all cursor-pointer">
                        ...
                        <div id="g_id_onload" data-client_id="YOUR_GOOGLE_CLIENT_ID.apps.googleusercontent.com"
                            data-context="signin" data-ux_mode="popup" data-callback="onGoogleAuthCallback"
    ...
    <script>
        window.cpmAjaxUrl = window.cpmAjaxUrl || "<?php echo esc_url($ajax_url); ?>";
        window.cpmNonce = window.cpmNonce || "<?php echo esc_js($nonce); ?>";
        var cpmAjaxUrl = window.cpmAjaxUrl;
        var cpmNonce = window.cpmNonce;
    ...
    <div class="cpm-auth-header-btn inline-flex items-center">
        <?php if ($is_logged_in): ?>
            <div class="flex items-center gap-3">
                <span class="text-xs md:text-sm font-bold text-slate-700">👋 Chào,
                    <?php echo esc_html($current_user->display_name); ?></span>
                <a href="<?php echo esc_url($logout_url); ?>"
                    class="bg-red-50 hover:bg-red-100 text-red-600 font-bold text-xs px-3 py-1.5 rounded-lg border border-red-200 transition-colors">
                    Đăng xuất
                </a>
            </div>
        <?php else: ?>
            <button type="button" onclick="openCpmAuthModal('login')"
                class="bg-blue-600 hover:bg-blue-700 text-white font-bold text-xs md:text-sm px-4 py-2 rounded-xl shadow-sm transition-all cursor-pointer border-none">
                🔑 Đăng nhập / Đăng ký
            </button>
        <?php endif; ?>
    </div>
